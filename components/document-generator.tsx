...
function BaylatDocument({ data }: { data: DocumentData }) {
  ...
  return <article className="baylat-document" id="baylat-print-document">
    ...
    <section className="document-body">
      ...
      <table className="transaction-table"><thead><tr><th>No</th><th>Description of goods</th><th>Quantity</th><th>Price per Quantity tonnes</th><th>N      K</th></tr></thead><tbody>{data.items.map((item, index) => <tr key={item.id}><td>{index + 1}</td><td>{item.description || '—'}</td><td>{item.quantity || 0} ({item.unit || '—'})</td><td>{formatCurrency(item.price)} X {item.quantity || 0}</td><td>{formatCurrency(itemAmount(item.quantity, item.price))}</td></tr>)}<tr className="total-row"><td colSpan={4}>GRANT TOTAL</td><td>{formatCurrency(total)}</td></tr></tbody></table>
...
export default function DocumentGenerator() {
  ...
  return <main className="generator-shell">
    ...
    <div><h1>Baylat Document Generator</h1><p>Internal Document Automation</p></div></div><button className="button button-outline" onClick={reset}><RotateCcw size={15} /> New document</button></header>
    <div className="workspace"><aside className="form-panel"><div className="panel-intro"><div><h2>Document details</h2></div></div>
      <section className="form-section"><div className="section-heading"><CalendarDays size={16} /><span>Document information</span></div><div className="form-grid"><label className="field"><span>Date</span><input type="date" value={data.date} onChange={(e) => update('date', e.target.value)} className="field-control" /></label><InputField label={fieldLabels.recipient} required value={data.recipient} onChange={(v) => update('recipient', v)} /><InputField label={fieldLabels.company} required value={data.company} onChange={(v) => update('company', v)} /><InputField label={fieldLabels.attention} value={data.attention} onChange={(v) => update('attention', v)} /><div className="field field-wide"><InputField label={fieldLabels.memo} required multiline value={data.memo} onChange={(v) => update('memo', v)} /></div></div></section>
      <section className="form-section"><div className="section-heading"><span>Transaction items</span><span className="section-total">₦{formatCurrency(total)}</span></div><div className="items-list">{data.items.map((item, index) => <div className="item-editor" key={item.id}><div className="item-topline"><span>Item {String(index + 1).padStart(2, '0')}</span><button className="icon-button danger" aria-label={`Remove item ${index + 1}`} onClick={() => setData((current) => ({ ...current, items: current.items.filter((row) => row.id !== item.id) }))}><Trash2 size={14} /></button></div><div className="item-grid"><label className="field item-description"><span>Description *</span><input className="field-control" value={item.description} onChange={(e) => setData((current) => ({ ...current, items: current.items.map((row) => row.id === item.id ? { ...row, description: e.target.value } : row) }))} /></label><label className="field"><span>Quantity
        *</span><input type="number" min="0" className="field-control" value={item.quantity} onChange={(e) => setData((current) => ({ ...current, items: current.items.map((row) => row.id === item.id ? { ...row, quantity: Number(e.target.value) } : row) }))} /></label><label className="field"><span>Unit</span><input className="field-control" value={item.unit} onChange={(e) => setData((current) => ({ ...current, items: current.items.map((row) => row.id === item.id ? { ...row, unit: e.target.value } : row) }))} /></label><label className="field"><span>Price *</span><input type="number" min="0" className="field-control" value={item.price} onChange={(e) => setData((current) => ({ ...current, items: current.items.map((row) => row.id === item.id ? { ...row, price: Number(e.target.value) } : row) }))} /></label></div><div className="item-amount">Line amount <strong>₦{formatCurrency(itemAmount(item.quantity, item.price))}</strong></div></div>)}</div><button className="add-item" onClick={() => setData((current) => ({ ...current, items: [...current.items, createEmptyItem()] }))}><Plus size={15} /> Add item</button></section>
      <section className="form-section"><div className="section-heading"><Building2 size={16} /><span>Account information</span></div><div className="form-grid"><InputField label={fieldLabels.accountName} required value={data.accountName} onChange={(v) => update('accountName', v)} /><InputField label={fieldLabels.accountNumber} required value={data.accountNumber} onChange={(v) => update('accountNumber', v)} /><InputField label={fieldLabels.bankName} required value={data.bankName} onChange={(v) => update('bankName', v)} /></div></section>
      {errors.length > 0 && <p className="error-message">Please complete the required fields: {errors.map((error) => error === 'items' ? 'at least one item' : error).join(', ')}.</p>}<div className="form-actions"><span id='info'>click on print button to get the pdf document*</span><button className="button button-ghost" onClick={reset}><X size={15} /> Clear</button>
      {/* <button className="button button-primary" onClick={validate}><Download size={15} /> Generate PDF</button> */}
      </div>
    </aside><section className="preview-panel"><div className="preview-toolbar"><div><span className="eyebrow">LIVE OUTPUT</span><h2>A4 document preview</h2></div><div className="toolbar-actions"><button className="icon-button" aria-label="Zoom out" onClick={() => setZoom((v) => Math.max(40, v - 5))}><Minus size={15} /></button><button className="zoom-label" onClick={() => setZoom(66)}>{zoom}%</button><button className="icon-button" aria-label="Zoom in" onClick={() => setZoom((v) => Math.min(100, v + 5))}><Plus size={15} /></button><button className="m_button button button-outline print-button" onClick={() => window.print()}><Printer size={15} /> Print</button></div></div><div className="preview-stage"><div className="paper-wrap" style={{ width: `${zoom}%` }}><BaylatDocument data={data} /></div></div></section></div>
  </main>
}
